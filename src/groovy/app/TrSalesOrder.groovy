...
class TrSalesOrder extends VerticalLayout{
	def selectedRow
	def itemlist
	...
	public TrSalesOrder() {
		...
		MenuBar.Command mycommand = new MenuBar.Command() {
			public void menuSelected(MenuItem selectedItem) {
				...
				{
					case "Add":
						def item = new BeanItem<SalesOrder>(tableContainer)
						windowAdd("Add");
					break
...
//		table.setPageLength(table.size())
			...
			void buttonClick(Button.ClickEvent event) {
				try{
					def object = [id:textId.getValue(),
								  ...
								  salesDate:salesDate.getValue(),
								  contact: comb.getValue()]
					
					if (object.id == "")
					{
						object =  Grails.get(SalesOrderService).createObject(object)
					}
					else
					{
						object =  Grails.get(SalesOrderService).updateObject(object)
					}				
					if (object.errors.hasErrors())
					{
						textCode.setData("code")
						salesDate.setData("salesDate")
						comb.setData("contact")
						Object[] tv = [textCode,salesDate,comb]
		...
		def saveButton = new Button("Save", new Button.ClickListener() {
			...
			void buttonClick(Button.ClickEvent event) {
				try{
					...
					if (object.id == "")
					{
						object =  Grails.get(SalesOrderDetailService).createObject(object)
					}
					else
					{
						object =  Grails.get(SalesOrderDetailService).updateObject(object)
					}
	...
	//@RequiresPermissions("Transaction:SalesOrder:Confirm")
			...
			ConfirmDialog.show(this.getUI(), caption + " ID:" + tableContainer.getItem(table.getValue()).getItemProperty("id") + " ? ",
				...
				public void onClose(ConfirmDialog dialog) {
					if (dialog.isConfirmed()) {
						...
						object = Grails.get(SalesOrderService).confirmObject(object)
						if (object.errors.hasErrors())
						{
							Object[] tv = [textId]
							generalFunction.setErrorUI(tv,object)
	...
	//@RequiresPermissions("Transaction:SalesOrder:Unconfirm")
		...
		if (currentUser.isPermitted(Title + Constant.AccessType.Unconfirm)) {
			ConfirmDialog.show(this.getUI(), caption + " ID:" + tableContainer.getItem(table.getValue()).getItemProperty("id") + " ? ",
				...
				public void onClose(ConfirmDialog dialog) {
					if (dialog.isConfirmed()) {
						def object = [id:tableContainer.getItem(table.getValue()).getItemProperty("id").toString()]
						object = Grails.get(SalesOrderService).unconfirmObject(object)
						if (object.errors.hasErrors())
						{
							Object[] tv = [textId]
							generalFunction.setErrorUI(tv,object)
	...
	//@RequiresPermissions("Transaction:SalesOrder:Delete")
		...
		if (currentUser.isPermitted(Title + Constant.AccessType.Delete)) {
			...
			new ConfirmDialog.Listener() {
				public void onClose(ConfirmDialog dialog) {
					if (dialog.isConfirmed()) {
						def object = [id:tableContainer.getItem(table.getValue()).getItemProperty("id").toString()]
						Grails.get(SalesOrderService).softDeleteObject(object)
						initTable()
					} else {
	...
	//@RequiresPermissions("Transaction:SalesOrder:Edit")
			...
			ConfirmDialog.show(this.getUI(), caption + " ID:" + tableDetailContainer.getItem(tableDetail.getValue()).getItemProperty("id") + " ? ",
				...
				public void onClose(ConfirmDialog dialog) {
					if (dialog.isConfirmed()) {
						...
						object = Grails.get(SalesOrderDetailService).softDeleteObject(object)
						if (object.errors.hasErrors())
						{
							Object[] tv = [textId]
							generalFunction.setErrorUI(tv,object)
	...
	private void windowEdit(def item,String caption) {
		if (currentUser.isPermitted(Title + Constant.AccessType.Edit)) {
			...
			salesDate = new DateField("Sales Date:")
			salesDate.setPropertyDataSource(item.getItemProperty("salesDate"))
			layout.addComponent(salesDate)
	...
	//@RequiresPermissions("Transaction:SalesOrder:Add")
	private void windowAdd(String caption) {
		if (currentUser.isPermitted(Title + Constant.AccessType.Add)) {
			...
			salesDate = new DateField("Sales Date:")
			layout3.addComponent(salesDate)
...
//		table.markAsDirtyRecursive();
	 ...
	 void initTable() {
		tableContainer = new BeanItemContainer<SalesOrder>(SalesOrder.class);
		//fillTableContainer(tableContainer);
	    itemlist = Grails.get(SalesOrderService).getList()
		...
		table.visibleColumns = ["code", "contact.name","salesDate","isConfirmed","confirmationDate","isDeleted","dateCreated","lastUpdated"]
...
//				print selectedRow
		...
		table.addValueChangeListener(new Property.ValueChangeListener() {
			public void valueChange(ValueChangeEvent event) {
				selectedRow = table.getValue()
				if (selectedRow != null) {
					tableDetailContainer = new BeanItemContainer<SalesOrderDetail>(SalesOrderDetail.class);
					def ind = tableContainer.getItem(table.getValue()).getItemProperty("id").toString()
					
					def itemListDetail = Grails.get(SalesOrderDetailService).getList(ind)
					tableDetailContainer.addNestedContainerProperty("item.description");
					tableDetailContainer.addNestedContainerProperty("item.id");
					tableDetailContainer.addNestedContainerProperty("salesOrder.id");
					...
					tableDetail.visibleColumns = ["code", "salesOrder.id","item.description","quantity","pendingDelivery","isConfirmed","confirmationDate","isDeleted","dateCreated","lastUpdated"]
