...
class SalesOrderValidationService {
	...
	{
		if (object.salesOrderDetails.findAll().size() == 0)
		{
	...
	def salesDateNotNull(def object)
	{
		if (object.salesDate == null)
		{
			object.errors.rejectValue('salesDate','null','PurchaseDate tidak boleh kosong')
		}
		return object
	}
	...
	def codeMustUnique(def object){
			def uniq = SalesOrder.findByCodeAndIsDeleted(object.code,false)
			if (uniq != null)
			{
	...
	def createObjectValidation(def object){
		...
		if (object.errors.hasErrors()) return object
		object = salesDateNotNull(object)
		return object
	}
